...
There are **three** main types of spatial statistics: 

- Geostatistics 
- Point Pattern Analysis
- Lattice Data Analysis

We'll focus on point pattern analysis.

## sp-land (detour)

We were previously working with sf objects, but we must revert back to sp objects
to utilize the functionality of many spatial stats libraries.
...
```{r geostats, echo=FALSE, fig.cap="Semivariogram modeling winter temps", out.width = "55%"}
...
```{r pointPattern, echo=FALSE, fig.cap="", out.width = "55%"}
knitr::include_graphics("https://upload.wikimedia.org/wikipedia/commons/c/c7/Snow-cholera-map.jpg")
```
...
[Tobler's First Law of Geography](https://en.wikipedia.org/wiki/Tobler%27s_first_law_of_geography) states:
...
- **Events** are points where the observation is made (assumes all points are known no missing values)
...
We don't want to just visually inspect events, we want to quantify and measure it...
...
The simplest study of measures in PPA is Centrography.

- **Mean Center**: the average X and Y values
...
<div class="notes">
basically descriptive stats for spatial features
</div>
...
if we look at the entire study area we can calculate point density as:
$$\frac{n}{a}$$
where $n$ is the observed number of events and $a$ is the study region's area. For
instance if we had 10 schools in 100 acres, our point density is $$\frac{10}{100} = 0.1$$
so 0.1 schools per acre.

## Complete Spatial Randomness

In PPA, the null hypothesis is complete spatial randomness (CSR), 
...
We looked at global density, but what about point patterns that can be measured 
across different sections within the study region?

We can then compare whether density is equally distributed across the region, 
or if there maybe an underlying local process at play.

But first, what does local mean?
...
Quadrats or sub-divisions is the simplest measure of local density.
...
```{r uberH3, echo=FALSE, out.width="50%",out.height="30%",fig.cap="Hexgons",fig.show='hold',fig.align='center'}
knitr::include_graphics(c("https://1fykyq3mdn5r21tpna3wkdyi-wpengine.netdna-ssl.com/wp-content/uploads/2018/06/image22-300x231.png",
                        "http://1fykyq3mdn5r21tpna3wkdyi-wpengine.netdna-ssl.com/wp-content/uploads/2018/06/image6-1.png"))
...
Distances from a triangle to its neighbors (left), a square to its neighbors (center), and a hexagon to its neighbors (right).
</div>


## Poisson Distribution

The probability distribution for a quadrat count of PPA is based off of the possion distribution. 

$$P(k)= \frac{\lambda^k e^{-\lambda}}{k!}$$
$$ P(k) = e^{-\lambda*a}$$

where $k$ is the event, $\lambda$ is the average intensity of the pattern per quadrant and $e$ is the base of the natural 
logarithm. it can also be calculated area in the second row.
...
You can measure the fit of the observed values from a quadrat count, by calculating variance squared over the mean and if they are equal. This is called the *variance-mean ratio* and if it equal 1.0, meaning it can be modeled by the Poisson distribution, it is randomly distributed. larger VMRs could suggest clumping, while smaller VMRs can show more uniformly dispersed events.  

This is also known as the Index of Dispersion. 

$$D = {\sigma^2 \over \mu }$$

## Pearson Chi-Square

Now for the $k$ number of events in a quadrat, you can derive $O$ the number of quadrats and $E$ the expected number from the poisson probability. 

Once you have the $O$, Poisson Probability, derived $E$, a chi square number can be created. 
...
```{r, echo=TRUE}
set.seed(1234)
sampleObs <- data.frame(trees = rnorm(25, mean = 5, sd = 2) %>% round())
sampleObs %>% head()
```
...
## Observed Values
```{r, echo=TRUE}
freqT <- table(sampleObs$trees) %>% data.frame() %>% 
    mutate_all(as.character) %>% mutate_all(as.numeric) %>% 
    rename(bins = Var1, observed = Freq)

freqT
```
## Expected Values
```{r, echo=TRUE}
expectedT <- data.frame(bins = seq(0, max(freqT$bins)),
                       expected = 100*dpois(seq(0, max(freqT$bins)), 
                                            lambda = mean(sampleObs$trees)))
expectedT
```
...
chiTable <- expectedT %>% 
    left_join(freqT) %>% 
    tidyr::replace_na(list(observed = 0)) %>% 
    mutate(diff = abs(observed-expected),
        chisq = (diff^2)/expected)

chiTable %>% summarize(observed = sum(observed),
                       diff = sum(diff),
                                   chisq = sum(chisq))
```
...
```{r chisqtable, echo=FALSE, fig.cap="", out.width="70%"}
knitr::include_graphics("https://i.stack.imgur.com/PbEqv.jpg")
```
...
```
 ...
 we expect around ~38 but observed 58.94 

<div class="notes">
As 58.94 < 38 in other words, the observed is much bigger than the expected, we would be rejecting the null hypothesis and would say that the observed distribution does not follow a random spatial distribution (it can be regular or aggregated).
</div>
...
## Quadrats in Nonuniform Geography

Quadrat areas don't have to be uniform like squares, they could
be created on a covariate like elevation, temperature, land uses, etc. or by 
creating a [tessellated surface](http://wiki.gis.com/wiki/index.php/Tessellation).
...
knitr::include_graphics("http://2rct3i2488gxf9jvb1lqhek9-wpengine.netdna-ssl.com/wp-content/uploads/2020/09/gispopsci_MAUPZone1.gif")
```
...
The assumption is that the statistical properties of the population do not change over time or space. 
The simplest form of a nonstationary data set is characterized by a mean that exhibits a trend in space or time

```{r nonStationarity, echo=FALSE, fig.cap="", out.width="60%"}
knitr::include_graphics("https://desktop.arcgis.com/en/arcmap/latest/tools/spatial-statistics-toolbox/GUID-35AE0DC2-C411-4DE4-9195-DD680D23CE93-web.png")
```


<div class="notes">
Notice that the relationship between the number of 911 calls and the number of people 
is different in high-population tracts and southwestern low-population tracts.
</div>
